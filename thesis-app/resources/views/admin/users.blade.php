                  ...
                  <div class="flex flex-wrap gap-2">
                    @foreach ($roles as $role)
                      @if (!$user->hasRole($role->name))
                        <form action="{{ route('admin.approveUser', ['id' => $user->id, 'role' => $role->name]) }}" method="POST">
                          @csrf
                          <button type="submit"
                            class="text-xs rounded-lg bg-gradient-to-r from-brand-500 to-indigo-600 text-white px-3 py-1 shadow hover:shadow-md">
                            Approve as {{ ucfirst($role->name) }}
                          </button>
                        </form>
                      @endif
                    @endforeach

                    <form action="{{ route('admin.denyUser', $user->id) }}" method="POST"
