...
<body class="min-h-screen bg-gradient-to-b from-brand-50 via-white to-white text-gray-900 antialiased">
  ...
  <header class="sticky top-0 z-30 bg-white/70 backdrop-blur border-b border-white/60">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
      ...
      <nav class="hidden md:flex items-center gap-6 text-sm font-medium">
        ...
        <a href="<?php echo e(route('community')); ?>" class="text-brand-700">Community</a>
  ...
  <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
    ...
    <?php if(session('success')): ?>
      <div class="p-4 bg-green-100 border border-green-300 rounded-xl text-green-800"><?php echo e(session('success')); ?></div>
    <?php endif; ?>
    <?php if($errors->any()): ?>
    ...
    <?php if (\Illuminate\Support\Facades\Blade::check('hasanyrole', 'admin|instructor')): ?>
    <div class="p-[2px] rounded-2xl bg-gradient-to-r from-pink-400 to-purple-500">
      <div class="bg-white rounded-2xl p-6">
        <form action="<?php echo e(route('community.store')); ?>" method="POST" class="space-y-3">
          <?php echo csrf_field(); ?>
    ...
    <section class="space-y-6">
      <?php $__empty_1 = true; $__currentLoopData = $posts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $post): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
          ...
          <div class="bg-white rounded-2xl p-6">
            <header class="flex flex-wrap items-center justify-between gap-2 mb-3">
              <div class="flex items-center gap-3">
                ...
                <div>
                  <h2 class="text-purple-700 font-semibold"><?php echo e($post->user->name); ?></h2>
              ...
              <div class="flex items-center gap-2">
                <button
                  data-edit-target="post-<?php echo e($post->id); ?>"
                  class="px-3 py-1 rounded-lg bg-yellow-500 text-white text-sm hover:bg-yellow-600"
                  type="button">✏️ Edit</button>
                <form action="<?php echo e(route('community.destroy', $post->id)); ?>" method="POST"
            ...
            <p class="text-gray-800 whitespace-pre-line"><?php echo e($post->content); ?></p>

            <!-- Reply form (everyone logged in) -->
            <div class="mt-5 border-t pt-4">
              <form action="<?php echo e(route('community.reply', $post->id)); ?>" method="POST" class="space-y-2">
                <?php echo csrf_field(); ?>
            ...
            <!-- Replies -->
            <?php if($post->replies && $post->replies->count()): ?>
                ...
                <?php $__currentLoopData = $post->replies; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $reply): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                  <div class="p-3 rounded-xl bg-gray-50">
                    <div class="flex items-center justify-between">
                      <p class="text-sm font-semibold text-purple-600"><?php echo e($reply->user->name); ?></p>
                    ...
                    <?php if($reply->user_id === auth()->id()): ?>
                      <div class="flex gap-2 mt-2">
                        <button
                          data-edit-target="reply-<?php echo e($reply->id); ?>"
                  ...
                  <!-- Reply Edit Modal -->
                  <div id="modal-reply-<?php echo e($reply->id); ?>" class="hidden fixed inset-0 z-40 bg-black/50 p-4">
                    <div class="mx-auto max-w-md rounded-2xl bg-white p-6 shadow-xl">
                      ...
                      <form action="<?php echo e(route('replies.update', $reply->id)); ?>" method="POST" class="space-y-3">
                        <?php echo csrf_field(); ?> <?php echo method_field('PUT'); ?>
                        <textarea name="content" rows="3" class="w-full p-3 border rounded-xl"><?php echo e($reply->content); ?></textarea>
                        <div class="flex justify-end gap-2">
                          <button type="button" data-close="#modal-reply-<?php echo e($reply->id); ?>"
        ...
        <!-- Post Edit Modal -->
        <?php if (\Illuminate\Support\Facades\Blade::check('hasanyrole', 'admin|instructor')): ?>
        <div id="modal-post-<?php echo e($post->id); ?>" class="hidden fixed inset-0 z-40 bg-black/50 p-4">
          <div class="mx-auto max-w-md rounded-2xl bg-white p-6 shadow-xl">
            <h3 class="text-lg font-bold text-purple-700 mb-3">Edit Post</h3>
            <form action="<?php echo e(route('community.update', $post->id)); ?>" method="POST" class="space-y-3">
              <?php echo csrf_field(); ?> <?php echo method_field('PUT'); ?>
              <textarea name="content" rows="4" class="w-full p-3 border rounded-xl"><?php echo e($post->content); ?></textarea>
              <div class="flex justify-end gap-2">
                <button type="button" data-close="#modal-post-<?php echo e($post->id); ?>"
                        class="px-4 py-2 rounded-xl bg-gray-200 hover:bg-gray-300">Cancel</button>
                <button type="submit" class="px-4 py-2 rounded-xl bg-green-600 text-white hover:bg-green-700">Save</button>
              </div>
            </form>
    ...
    <?php if(method_exists($posts, 'links')): ?>
      <div class="pt-4"><?php echo e($posts->links()); ?></div>
    <?php endif; ?>
  </main>
...
<?php /**PATH C:\Users\Marc\Documents\GitHub\THESIS-FILES\thesis-app\resources\views/community.blade.php ENDPATH**/ ?>