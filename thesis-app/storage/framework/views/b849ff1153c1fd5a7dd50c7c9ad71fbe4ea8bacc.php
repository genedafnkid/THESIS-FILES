...
<body class="min-h-screen bg-gradient-to-b from-brand-50 via-white to-white text-gray-900 antialiased">
  ...
  <header class="sticky top-0 z-30 bg-white/70 backdrop-blur border-b border-white/60">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
      ...
      <nav class="hidden md:flex items-center gap-6 text-sm font-medium">
        <a href="<?php echo e(route('modules.index')); ?>" class="text-brand-700 font-semibold">Modules</a>
  ...
  <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
    ...
    <?php if(session('success')): ?>
      <div class="rounded-xl border border-green-200 bg-green-50 px-4 py-3 text-green-800"><?php echo e(session('success')); ?></div>
    <?php endif; ?>
    <?php if($errors->any()): ?>
    ...
    <?php if (\Illuminate\Support\Facades\Blade::check('hasanyrole', 'instructor|admin')): ?>
    <section class="p-[2px] rounded-2xl bg-gradient-to-r from-pink-500 to-purple-600">
      <div class="bg-white rounded-2xl p-6 sm:p-8">
        ...
        <form action="<?php echo e(route('modules.store')); ?>" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <?php echo csrf_field(); ?>
        ...
        <article class="p-[2px] rounded-2xl bg-gradient-to-r from-pink-400 to-purple-500">
          <div class="bg-white rounded-2xl p-6 sm:p-8">
            <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
              <div>
                <h3 class="text-2xl font-bold text-purple-700"><?php echo e($module->title); ?></h3>
                <p class="mt-1 text-gray-600"><?php echo e($module->description); ?></p>
              </div>
              <div class="flex gap-2">
                <?php if($module->file_path): ?>
                  <a href="<?php echo e(asset('storage/' . $module->file_path)); ?>" target="_blank"
            ...
            <div class="mt-5 flex flex-wrap gap-3">
              <button
                type="button"
                onclick="document.getElementById('edit-form-<?php echo e($module->id); ?>').classList.toggle('hidden')"
            ...
            <!-- Hidden Edit Form -->
            <div id="edit-form-<?php echo e($module->id); ?>" class="hidden mt-6 rounded-2xl border border-gray-200 bg-gray-50 p-5">
              <form action="<?php echo e(route('modules.update', $module->id)); ?>" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <?php echo csrf_field(); ?> <?php echo method_field('PUT'); ?>
                <div class="sm:col-span-2">
                  <label class="block text-sm font-semibold text-gray-700 mb-1">Title</label>
                  <input type="text" name="title" value="<?php echo e($module->title); ?>"
                         class="w-full rounded-xl border-gray-300 px-3 py-2 focus:border-brand-500 focus:ring-brand-500" required>
                </div>
                <div class="sm:col-span-2">
                  <label class="block text-sm font-semibold text-gray-700 mb-1">Description</label>
                  <textarea name="description" rows="3" class="w-full rounded-xl border-gray-300 px-3 py-2 focus:border-brand-500 focus:ring-brand-500" required><?php echo e($module->description); ?></textarea>
                </div>
                <div>
                  <label class="block text-sm font-semibold text-gray-700 mb-1">Replace File (optional)</label>
                  <input type="file" name="file" class="w-full rounded-xl border-gray-300 px-3 py-2">
                  <?php if($module->file_path): ?>
                    <p class="text-xs text-gray-500 mt-1">Current: <?php echo e(basename($module->file_path)); ?></p>
                  <?php endif; ?>
                </div>
                <div class="flex items-end">
  ...
  <footer class="mt-10 mb-8 text-center text-sm text-gray-500">
    © <?php echo e(date('Y')); ?> Digital Theology Classroom
  </footer>
</body>
</html>
<?php /**PATH C:\Users\Marc\Documents\GitHub\THESIS-FILES\thesis-app\resources\views/modules.blade.php ENDPATH**/ ?>