...
<head>
  ...
  <script>
    ...
    window.currentUser = { id: "<?php echo e(auth()->id()); ?>" };
  </script>
</head>
<body class="min-h-screen bg-gradient-to-b from-brand-50 via-white to-white text-gray-900 antialiased">
  ...
  <header class="sticky top-0 z-30 bg-white/70 backdrop-blur border-b border-white/60">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
      ...
      <nav class="hidden md:flex items-center gap-6 text-sm font-medium">
        ...
        <a href="<?php echo e(route('faith-room')); ?>" class="text-brand-700">Faith Room</a>
      </nav>
  ...
  <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="p-[2px] rounded-2xl bg-gradient-to-r from-pink-400 to-purple-500">
      <div class="bg-white rounded-2xl p-3 sm:p-4">
        <!-- Loading / Fallback -->
        <div id="loader" class="mb-3 flex items-center justify-between rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-amber-800 text-sm">
          <span>Loading the game… If it takes too long, try Reload or open in a new tab.</span>
          <a target="_blank" rel="noopener"
             href="<?php echo e(asset('webgl-1/index.html')); ?>"
        ...
        <div class="relative w-full rounded-xl overflow-hidden shadow">
          <div class="w-full h-0 pb-[56.25%] bg-gray-100" id="iframe-holder">
            <iframe
              ...
              src="<?php echo e(asset('webgl-1/index.html')); ?>"
  ...
  <script>
    // Re-expose in case the game reloads the page inside the iframe and looks at parent:
    window.currentUser = { id: "<?php echo e(auth()->id()); ?>" };

    // Save score API the Unity content can call via parent.__unitySaveScore(score)
    window.__unitySaveScore = async function (score) {
      ...
      try {
        const res = await fetch("/scores", {
          ...
          body: JSON.stringify({ score: score, user_id: window.currentUser.id })
        });
        ...
        try {
          ...
          console.warn("ℹ️ Non-JSON response:", text);
        }
      } catch (err) {
...
<?php /**PATH C:\Users\Marc\Documents\GitHub\THESIS-FILES\thesis-app\resources\views/play1.blade.php ENDPATH**/ ?>