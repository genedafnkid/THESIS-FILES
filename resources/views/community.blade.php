    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <section class="space-y-6">
            @forelse ($posts as $post)
                <article class="p-[2px] rounded-2xl bg-gradient-to-r from-pink-300 to-purple-400">
                    <div class="bg-white rounded-2xl p-6">
                        <header class="flex items-center justify-between mb-4">
                            ...
                            <!-- Edit/Delete visible to post owner OR admin -->
                            @auth
                                @if(auth()->id() === $post->user_id || auth()->user()->hasRole('admin'))
                                    <div class="flex items-center gap-2">
                                        ...
                                        <form action="{{ route('community.destroy', $post->id) }}" method="POST"
                                            ...
                                            @csrf
                                            @method('DELETE')
                                            <button class="px-3 py-1 rounded-lg bg-red-500 text-white text-sm hover:bg-red-600"
                                                type="submit">
                                                🗑️ Delete
                                            </button>
                                        </form>
                        ...
                        <div class="mt-5 border-t pt-4">
                            @auth
                                <form action="{{ route('community.reply', $post->id) }}" method="POST" class="space-y-2">
                                    ...
                                    <div class="flex justify-end">
                                        <button class="px-4 py-2 rounded-xl bg-blue-600 text-white hover:bg-blue-700" type="submit">
                                            Reply
                                        </button>
                                    </div>
                                </form>
                            @else
                                <p class="text-sm text-gray-500">Please <a href="{{ route('login') }}"
                                        class="text-purple-600 font-medium">log in</a> to reply.</p>
                            @endauth
                        </div>
                                                ...
                                                @if ($reply->user_id === auth()->id())
                                                    <div class="flex gap-2 mt-2">
                                                        ...
                                                        <form action="{{ route('replies.destroy', $reply->id) }}" method="POST"
                                                            ...
                                                            @method('DELETE')
                                                            <button
                                                                class="px-2 py-1 text-xs rounded-lg bg-red-500 text-white hover:bg-red-600"
                                                                type="submit">
                                                                🗑️ Delete
                                                            </button>
                                                        </form>
                                        ...
                                        <div class="mx-auto max-w-md rounded-2xl bg-white p-6 shadow-xl">
                                            ...
                                            <form action="{{ route('replies.update', $reply->id) }}" method="POST" class="space-y-3">
                                                ...
                                                <textarea name="content" rows="3"
                                                    class="w-full p-3 border rounded-xl">{{ $reply->content }}</textarea>
                ...
                <!-- Post Edit Modal (only owner or admin) -->
                @auth
                    @if(auth()->id() === $post->user_id || auth()->user()->hasRole('admin'))
                            ...
                            <div class="mx-auto max-w-md rounded-2xl bg-white p-6 shadow-xl">
                                ...
                                <form action="{{ route('community.update', $post->id) }}" method="POST" class="space-y-3">
                                    ...
                                    <textarea name="content" rows="4"
                                        class="w-full p-3 border rounded-xl">{{ $post->content }}</textarea>
                                    <div class="flex justify-end gap-2">
